...
class WaypointGuiNode(Node):
    ...
    def __init__(self, signal_bridge: SignalBridge):
        ...
        self.waypoint_data_file = self.get_parameter('waypoint_data_file').get_parameter_value().string_value
        if self.waypoint_data_file:
            self.get_logger().info(f'Waypoint data file: {self.waypoint_data_file}')
        else:
            self.get_logger().error(
                'waypoint_data_file is empty; check that the shared parameter file was loaded'
            )
...
class WaypointGuiWindow(QMainWindow):
    ...
    def __init__(self, ros_node: WaypointGuiNode, signal_bridge: SignalBridge):
        ...
        map_file = self.ros_node.waypoint_data_file
        if map_file and os.path.isfile(map_file):
            try:
                ...
                self.ros_node.get_logger().info(
                    f'Loaded satellite map from {map_file} '
                    f'({self.pix_gps_map.width}x{self.pix_gps_map.height})'
                )
            except Exception as e:
                self.ros_node.get_logger().error(
                    f'Failed to load satellite map from {map_file}: {e}'
                )
        elif map_file:
            self.ros_node.get_logger().error(f'Waypoint data file not found: {map_file}')
